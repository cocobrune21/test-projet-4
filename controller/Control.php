...
{
    $db = dbConnect();
    $req = $db->query('SELECT id, title, script, DATE_FORMAT date_post_episode FROM episodes ORDER BY date_post_episode DESC LIMIT 0, 5');
...
{
    $db = dbConnect();
    $req = $db->prepare('SELECT id, title, script, DATE_FORMAT date_post_episode FROM posts WHERE id = ?');
